...
/**
 * Function to update selection badge based on selected values
 * @param {Array} selectedValues - Array of selected values
 * @param {HTMLElement} badgeElement - The badge element to update
 * @param {string} dropdownListId - The ID of the dropdown list
 */
export function updateSelectionBadge(selectedValues, badgeElement, dropdownListId) {
    if (selectedValues.length > 0) {
        ...
        // Add 'has-selection' class to dropdown-header for styling
        ...
        // Remove 'has-selection' class from dropdown-header
        const dropdownHeader = document.querySelector(`.dropdown-header[data-target="${dropdownListId}"]`);
        if (dropdownHeader) {
...
export function populateDropdown(dropdownListId, options, selectedValues = [], singleSelect = false, showCounts = true) {
    ...
    updateSelectionBadge(selectedValuesUpdated, selectionBadge, dropdownListId);
...
export function updateFilters(page = 1) {
    ...
    fetch(`/filter_movies?${params.toString()}`)
        ...
        .then(data => {
            ...
            // Update the visibility of filter navigation arrows
            updateFilterNavigationArrowsVisibility();
        })
        .catch(error => {
            console.error('Error fetching filter data:', error);
            if (movieContainer) {
                movieContainer.innerHTML = `<p class="no-movies-message">An error occurred. Bitte versuche es später erneut.</p>`;
            }
        })
        .finally(() => {
